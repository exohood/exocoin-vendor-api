...
namespace Exohood\Vendor;

use Exohood\Vendor\Exception\VendorException;
...
class Vendor
{
     ...
     */
    const URL_POLL = 'https://poll.exohood.com/vendor/check-payment';

    /**
     * Url for the exchange rate JSON.
     */
    const URL_SUPPLY = 'https://supply.exohood.com/app-value-v2.json';
     ...
     * @var array
     *
     * Currencies accepted for converting to EXO.
     */
    ...
    /**
     * Instantiate a new Exohood vendor client.
     *
     * @param string $apiKey
    ...
    /**
     * Convert a local currency amount to EXO.
     *
     * @param $value
     * @param $currency
     * @return float|string
     * @throws VendorException
     */
    public function currencyToEXO($value, $currency)
    {
        ...
        if (extension_loaded('bcmath')) {
            ...
            $this->EXO = bcdiv($value, $rate, 2);
        } else {
            $this->EXO = number_format((float)$value / $rate, 2, '.', '');
        }
    ...
    public function getQrCode($amount, $outlet, $paymentId = null)
    {
        ...
        return sprintf('EXO-it-%s/%s/%.2f', $this->outlet, $this->paymentId, $this->EXO);
    }
    ...
    public function getQr($amount, $currency, $outlet, $paymentId = null)
    {
        ...
        $EXO = $this->currencyToEXO($amount, $currency);

        // Build the QR Code string.
        $qrCode = $this->getQrCode($EXO, $outlet, $paymentId);
